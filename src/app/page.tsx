...
export default function HomePage() {
  ...
  async function fetchProducts() {
    try {
      ...
      data.forEach((p: Product) => {
        const first = p.stock.find((s) => s.availableStock > 0);
  ...
  async function handleReserve(product: Product) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.status === 409) {
        toast.error(data.error || "Not enough stock. Please choose another warehouse.");
        fetchProducts();
        return;
      }
      if (!res.ok) {
        ...
        {loading ? (
          ...
          >
            {products.map((product) => {
              ...
              const availableCount = stockForSelected?.availableStock ?? 0;
                        ...
                        {product.stock.map((s) => (
                          <div key={s.warehouseId} className="space-y-1">
                            <div className="flex justify-between items-center">
                              ...
                              >
                                {s.availableStock}/{s.totalUnits}
                              </span>
                            </div>
                            <StockBar available={s.availableStock} total={s.totalUnits} />
                          </div>
                        ))}
                      </div>
                    </CardContent>
